...
| D20 | 8.9 | 生活类 demo：加「AI 旅行规划师」端到端 session（init→问偏好→搜目的地→排行程→算预算→重排自环→done），展示对非开发者场景的通用性（v0.5.13） | ✅ |
| D21 | 8.9 | 默认模型切到 qwen3.7-flash + proxy 层硬禁用 qwen3.6-flash（请求即 400）；playground 下拉默认与历史筛选同步（v0.5.14） | ✅ |
