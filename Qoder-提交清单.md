...
> 截止：2026-8-9 ｜ 当前代码版本 v0.5.16（D24 代理层加固：SSE 事件计数 + 终态写库 try 保护 + end/error 竞态短路；Railway 需手动 Redeploy 拉取）
...
1. **Railway 重新部署（待你点）**
   - 当前 main 已到 **v0.5.16**（D24，含 D23「回放建议」+ D24 代理层加固）。公网实例仍为旧版本（Railway auto-deploy 关闭，需手动 Redeploy，已在 D15/D19 实测确认）。
   - 去 Railway 控制台对 `lobster-tracer` 点 **Redeploy**，拉取最新 main；验证 `/health` version=`0.5.16`，会话列表出现「AI 旅行规划师：帮我安排 5 天东京游」，且聚合面板出现「💡 回放建议」卡片。
...
3. **同步更新使用手册版本号（已完成）**
   - `使用手册.md` 已同步到 **v0.5.16** / D24，版本一致，评委不会看到版本错配。
...
| 作品使用手册 | ✅ | `使用手册.md`，已同步 v0.5.16（含 30 秒体验指南） |
