...
> ✅ 公网 demo（Railway）已开启 **auto-deploy**：push 到 `main` 即自动拉取最新构建，无需手动 Redeploy。代码已到 v0.5.20。

---

## v0.5.20 · D28+D29 · REG-01 修复 + 版本号统一

### 🐛 修复（审计 REG-01）
- **REG-01 · smoke 测试断言位置错误**：D26 在 seed 之前放置 `suggestions` 非空断言，干净环境（CI / 全新克隆）必失败。D28 将非空断言移至步骤 4c（seed 之后），步骤 2 只保留类型断言。修复后 22/22 断言全过，零功能代码变更。

### 🔧 优化（版本号统一）
- **版本号统一 v0.5.20**：`package.json`、`/health` 接口、README badge 从 v0.5.19 / v0.5.14 统一更新为 v0.5.20。

### 为什么这事重要（评委视角）
- 修复前评委克隆仓库跑 `npm test` 会直接看到红色失败。修复后干净环境也能一次通过，不留「测试不过」的第一印象。
